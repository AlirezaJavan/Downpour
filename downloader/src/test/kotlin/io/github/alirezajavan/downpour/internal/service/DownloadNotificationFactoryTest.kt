...
class DownloadNotificationFactoryTest {
    ...
    fun `build creates notification for active downloads`() {
        val item = mockk<io.github.alirezajavan.downpour.api.DownloadItem>(relaxed = true)
        every { item.id } returns "id"
        every { item.state } returns io.github.alirezajavan.downpour.api.DownloadState.Queued
