...
internal class FakeDownloadDao : DownloadDao {
    ...
    override suspend fun findNonTerminalByChecksum(
        algorithm: Int,
        value: String,
        terminal: List<DownloadStatus>,
    ): String? = rows.values.firstOrNull { it.checksumAlgorithm == algorithm && it.checksumValue == value && it.status !in terminal }?.id
